...
int unpackageDataFromNetwork(char *packet, int packetlen, char *databuf, int *databuflen) {
	...
	unsigned char *hash;
	unsigned char correcthash[DATAPACKET_HASHSIZE];
	...
	memcpy(correcthash, cdata->hash, DATAPACKET_HASHSIZE);
	memset(cdata->hash, 0, DATAPACKET_HASHSIZE);
	hash = SHA1(packagebuf1, buf1len, NULL);
	if(memcmp(hash, correcthash, (DATAPACKET_HASHSIZE > SHA_DIGEST_LENGTH) ? SHA_DIGEST_LENGTH : DATAPACKET_HASHSIZE) != 0) {
...
int packageDataForNetwork(char *data, int datalen, char *netbuf, int *netbuflen, int nodeid) {
	...
	*(struct datapacket_cryptdata *)packagebuf2 = cdata;
	...
	// Calculate the hash
	hash = SHA1(packagebuf2, buf2len, NULL);
	memcpy(((struct datapacket_cryptdata *)packagebuf2)->hash, hash, (DATAPACKET_HASHSIZE > SHA_DIGEST_LENGTH) ? SHA_DIGEST_LENGTH : DATAPACKET_HASHSIZE);
